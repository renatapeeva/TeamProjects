...
public class ProjectTopicController {
    ...
    @GetMapping("/topic/details/{index}")
    public String detailsPage(@PathVariable String index, Model model,HttpSession session) {
        Optional<Student> student=studentService.getStudentById(index);
        Student student1=student.get();
        model.addAttribute("student", session.getAttribute("student"));
        Optional<ProjectTopic> topic=projectTopicService.findByStudentsContaining(student1);
        if(topic.isEmpty())
            return "noTopicEnrolled.html";
        Long id=topic.get().getId();
        return "redirect:/topics/" + id + "/details";
    }
    @GetMapping("/topic/professor/{id}")
    public String professorTopics(@PathVariable String id, Model model,HttpSession session) {
        Professor professor=professorService.getProfessorById(id);
        List<ProjectTopic> topics=projectTopicService.findAllByProfessor(professor);
